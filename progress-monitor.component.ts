// progress-monitor.component.ts — Suivi en temps réel du clonage
    ...
    <div class="col-lg-9">
      ...
      <div class="d-flex align-items-center gap-3 mb-4">
        <h1 class="h3 text-white mb-0">
          <i class="bi bi-activity text-info me-2"></i>Progression du clonage
        </h1>
      ...
      <div class="card bg-dark border-info mb-4">
        <div class="card-header border-info d-flex justify-content-between align-items-center">
          <span class="text-info fw-bold">
            <i class="bi bi-hdd me-1"></i>Opération en cours
          </span>
          <span class="text-secondary small" *ngIf="status">
            Mis à jour il y a {{ lastUpdateSec }}s
          </span>
        </div>
        <div class="card-body">
          ...
          <div *ngIf="status && status.status !== 'idle'">
            ...
            <div class="alert mb-4"
              ...
              <div class="mt-1 text-danger" *ngIf="status.error_message">
                Détail : {{ status.error_message }}
              </div>
            </div>
            ...
            <div class="mb-2 d-flex justify-content-between">
              ...
              <span class="text-secondary">
                {{ formatBytes(status.bytes_done) }} / {{ formatBytes(status.bytes_total) }}
              </span>
            </div>
            ...
            <div class="row g-3 mb-3">
              ...
              <div class="col-6 col-md-3">
                <div class="card bg-secondary bg-opacity-10 border-secondary text-center p-3">
                  ...
                  <small class="text-secondary">Restant estimé</small>
                </div>
              </div>
              <div class="col-6 col-md-3">
                <div class="card bg-secondary bg-opacity-10 border-secondary text-center p-3">
            ...
            <!-- Graphique de vitesse simplifié -->
            <div class="card bg-black border-secondary p-3 mb-3">
              <small class="text-secondary mb-2 d-block">Historique de vitesse (Mo/s)</small>
              <div class="d-flex align-items-end gap-1" style="height:50px;">
                <div *ngFor="let s of speedHistory"
                     class="bg-info rounded-top"
                     [style.height.%]="s / maxSpeed * 100"
                     [style.flex]="1"
                     [title]="s.toFixed(1) + ' Mo/s'">
                </div>
              </div>
            </div>
            ...
            <div class="d-flex gap-2 justify-content-end">
              ...
              <button class="btn btn-outline-info" *ngIf="status.status === 'done'" routerLink="/clone">
                <i class="bi bi-copy me-1"></i>Nouveau clonage
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="card bg-dark border-secondary">
        <div class="card-header border-secondary d-flex justify-content-between align-items-center">
          <h6 class="mb-0 text-secondary">
            <i class="bi bi-terminal me-2"></i>Journal en direct
          </h6>
        ...
        <div class="card-body p-0">
          <pre class="bg-black text-success p-3 mb-0 small"
               style="max-height:200px;overflow-y:auto;font-size:0.78rem;"
               #logBox>{{ logLines.join('') || '(pas encore de logs)' }}</pre>
        </div>
      </div>
...
export class ProgressMonitorComponent implements OnInit, OnDestroy {
  ...
  logLines:    string[] = [];
  speedHistory: number[] = Array(30).fill(0);
  maxSpeed     = 1;
  cancelling   = false;
  lastUpdateSec = 0;
  ...
  ngOnInit() {
    this.pollSub = this.api.pollStatus(1000).subscribe(s => {
      this.status = s;
      this.lastUpdateSec = 0;

      // Historique de vitesse
      this.speedHistory.push(s.speed_mbps);
      this.speedHistory = this.speedHistory.slice(-30);
      this.maxSpeed = Math.max(1, ...this.speedHistory);
    });

    // Compteur "mis à jour il y a Ns"
    this.tickSub = setInterval(() => this.lastUpdateSec++, 1000);
    this.loadLogs();
  ...
  loadLogs() {
    this.api.getLogs(50).subscribe(r => {
