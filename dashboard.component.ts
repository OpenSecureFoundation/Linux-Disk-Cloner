...
<div class="container-fluid py-4">
  ...
  <div class="alert alert-info d-flex align-items-center mb-4" *ngIf="health?.mock_mode">
    ...
    <div>
      <strong>Mode simulation actif.</strong> libclone.so non trouvé — les disques affichés sont fictifs.
      Sur OpenIndiana, compilez d'abord le moteur C avec <code>make</code> dans le dossier <code>engine/</code>.
    </div>
  </div>
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-header d-flex justify-content-between align-items-center border-secondary">
      <h5 class="mb-0 text-white">
        <i class="bi bi-hdd-stack text-info me-2"></i>Disques raw détectés <span class="text-secondary fs-6">(/dev/rdsk/)</span>
      </h5>
    ...
    <div class="card-body p-0">
        ...
        <table class="table table-dark table-hover align-middle mb-0">
          ...
          <tbody>
            <tr *ngFor="let disk of disks">
              <td class="ps-3">
                <div class="d-flex align-items-center gap-2">
                  <i class="bi bi-hdd fs-5"
                     ...
                     [class.text-info]="!disk.is_mounted && !disk.is_system_disk">
                  </i>
                  <div>
              ...
              <td>
                <span class="badge bg-secondary fs-6">{{ disk.size_human }}</span>
              </td>
              <td>
                <span class="badge" [class]="disk.is_mounted ? 'bg-warning text-dark' : 'bg-success'">
                  <i class="bi me-1" [class]="disk.is_mounted ? 'bi-lock-fill' : 'bi-check'"></i>
                  {{ disk.is_mounted ? 'Monté' : 'Libre' }}
                </span>
              </td>
              <td>
                <span class="font-monospace text-secondary small" *ngIf="disk.mount_point">
                  {{ disk.mount_point }}
                </span>
                <span class="text-secondary" *ngIf="!disk.mount_point">—</span>
              </td>
              <td class="text-center">
                <span class="badge bg-danger" *ngIf="disk.is_system_disk">
                  <i class="bi bi-shield-lock-fill me-1"></i>Système
                </span>
                <span class="text-success" *ngIf="!disk.is_system_disk">
              ...
              <td class="text-center pe-3">
                <button class="btn btn-outline-info btn-sm"
                        ...
                        title="{{ disk.is_system_disk ? 'Disque système — clonage interdit' : 'Utiliser comme source' }}">
      ...
      <div class="text-center py-5" *ngIf="!loading && disks.length === 0">
        ...
        <p class="text-secondary mt-3">Aucun disque raw détecté dans /dev/rdsk/</p>
      </div>
    </div>
  </div>
...
export class DashboardComponent implements OnInit {
  ...
  refresh() {
    ...
    this.api.getDisks().subscribe({
      ...
      error: e => {
        this.errorMsg = `Impossible de contacter l'API Flask (${e.status || 'réseau'}). Vérifiez que le backend tourne sur le port 5000.`;
        this.loading = false;
      },
    });
  }
